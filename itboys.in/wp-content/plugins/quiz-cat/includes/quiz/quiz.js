...
jQuery( document ).ready(function($) {
	...
	function fca_qc_update_timer( q, endDate ){
		...
		if( Date.now() < endDate ){ 
			...
			if( hours_remaining ){
				$( quiz.selector ).find( '.fca_qc_timer.fca_qc_hours').html( hours_remaining + ' :' )
			}
			$( quiz.selector ).find( '.fca_qc_timer.fca_qc_minutes').html( minutes_remaining + ' :' )
			$( quiz.selector ).find( '.fca_qc_timer.fca_qc_seconds').html( seconds_remaining )
			...
			$( quiz.selector ).find( '.fca_qc_timer.fca_qc_minutes').html( '0 :' )
			$( quiz.selector ).find( '.fca_qc_timer.fca_qc_seconds').html( '0.00' )
