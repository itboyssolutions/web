...
function fca_qc_add_image_input($img = '', $name = '', $id = '', $hidden = false) {
	...
	$html .= "<button $hidden title='" . esc_attr__('Adds an image (optional).  For best results, use images at least 250px wide and use the same image resolution for each image you add to an answer.', 'quiz-cat') . "' type='button' class='button-secondary fca_qc_quiz_image_upload_btn'>" . esc_attr__('Add Image', 'quiz-cat') . "</button>";
	...
	$html .= "<div class='fca_qc_image_hover_controls'>";
		...
		if ( empty ( $img ) ) {
			$html .= "<button type='button' class='button-secondary fca_qc_quiz_image_change_btn' $hidden>" . esc_attr__('Change', 'quiz-cat') . "</button>";
			$html .= "<button type='button' class='button-secondary fca_qc_quiz_image_revert_btn' $hidden>" . esc_attr__('Remove', 'quiz-cat') . "</button>";
		}else {
			$html .= "<button type='button' class='button-secondary fca_qc_quiz_image_change_btn'>" . esc_attr__('Change', 'quiz-cat') . "</button>";
			$html .= "<button type='button' class='button-secondary fca_qc_quiz_image_revert_btn'>" . esc_attr__('Remove', 'quiz-cat') . "</button>";
		}
...
function fca_qc_input ( $name = '', $placeholder = '', $value = '', $type = 'text', $atts = '' ) {
	...
	switch ( $type ) {
		...
		case 'shortcode':
			$html = "<input $atts type='text' placeholder='$placeholder' class='fca_qc_input_wide fca_qc_" . $type . "_input' name='fca_qc_" . $type . "_input' value='$value'></input>";
			break;

		case 'textarea':
			$html = "<td><textarea placeholder='$placeholder' class='fca_qc_question_texta $atts'>$value</textarea></td>";
			break;

		case 'hidden':
			$html = "<input $atts type='hidden' class='fca-qc-input-$type fca-qc-$name' name='fca_qc[$name]' value='$value'>";
			break;
		...
		default:
			$html = "<input $atts type='$type' placeholder='$placeholder' class='fca-qc-input-$type fca-qc-$name' name='fca_qc[$name]' value='$value'>";
	}

	return $html;
}

function fca_qc_mailing_list( $name, $list ){

	$list = esc_attr( $list );

	$html = "<th>";
		$html .= "<label class='fca_qc_admin_label fca_qc_admin_settings_label' for='fca_qc_$name'>" . esc_attr__('List to subscribe to', 'quiz-cat') . "</label>";
	$html .= "</th>";
	
	$html .= "<td>";
		
		$html .= '<select class="fca_qc_input_wide" id="fca_qc_' . $name . '" name="fca_qc_' . $name . '">';
			if ( !empty ( $list ) ) {
				$html .= "<option value='$list' selected='selected' >" . esc_attr__('Loading...', 'quiz-cat') . "</option>";
			}						
		$html .= '</select>';
		...
		$html .= "<div class='fca-wysiwyg-nav' style='display:none'>";
			...
			$html .= '<div class="fca-wysiwyg-group fca-wysiwyg-link-group">';
				...
				$html .= '<div class="fca-wysiwyg-url-dialog" data-wysihtml5-dialog="createLink" style="display: none">';
					...
					$html .= '<a class="button button-secondary" data-wysihtml5-dialog-action="cancel">' . esc_attr__('Cancel', 'quiz-cat') . '</a>';
					$html .= '<a class="button button-primary" data-wysihtml5-dialog-action="save">' . esc_attr__('OK', 'quiz-cat') . '</a>';
				$html .= "</div>";
			$html .= "</div>";
